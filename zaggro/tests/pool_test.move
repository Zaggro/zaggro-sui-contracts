...
module zaggro::pool_tests {
    ...
    /// Init a Pool with a 1_000_000 ZAGGRO_COIN and 1_000_000_000 SUI;
    /// Set the ratio ZAGGRO_COIN : SUI = 1 : 1000.
    ...
    fun test_init_pool_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            let pool = test::take_shared<Pool<POOLEY, ZAGGRO_COIN>>(test);
    ...
    fun test_add_liquidity_(test: &mut Scenario) {
        ...
        next_tx(test, theguy);
        {
            ...
            let lsp_tokens = pool::add_liquidity(
                ...
                mint<ZAGGRO_COIN>(amt_tok, ctx(test)),
                ctx(test)
            );
    ...
    /// The other guy tries to exchange 5_000_000 sui for ~ 5000 ZAGGRO_COIN,
    ...
    /// The owner swaps back ZAGGRO_COIN for SUI and expects an increase in price.
    /// The sent amount of ZAGGRO_COIN is 1000, initial price was 1 ZAGGRO_COIN : 1000 SUI;
    fun test_swap_tok_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            ...
            let sui = pool::swap_token(pool_mut, mint<ZAGGRO_COIN>(1000, ctx(test)), ctx(test));
    ...
    fun test_withdraw_almost_all_(test: &mut Scenario) {
        ...
        // someone tries to pass (MINTED_LSP - 1) and hopes there will be just 1 ZAGGRO_COIN
        next_tx(test, owner);
        {
            let lsp = mint<LSP<POOLEY, ZAGGRO_COIN>>(31622000 - 1, ctx(test));
            let pool = test::take_shared<Pool<POOLEY, ZAGGRO_COIN>>(test);
            let pool_mut = &mut pool;
            ...
            assert!(tok_reserve > 0, 3); // actually 1 ZAGGRO_COIN is left
    ...
    fun test_withdraw_all_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            let lsp = mint<LSP<POOLEY, ZAGGRO_COIN>>(31622000, ctx(test));
            let pool = test::take_shared<Pool<POOLEY, ZAGGRO_COIN>>(test);
            let pool_mut = &mut pool;
